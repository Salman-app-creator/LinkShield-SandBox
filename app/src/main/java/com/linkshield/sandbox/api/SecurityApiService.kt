...
class SecurityApiService {
    ...
    suspend fun checkUrl(url: String): ThreatCheckResult = withContext(Dispatchers.IO) {
        if (url.isBlank()) {
        ...
        try {
            val result = checkWithGoogleSafeBrowsing(url.trim())
    ...
    private fun checkWithGoogleSafeBrowsing(url: String): ThreatCheckResult? {
        ...
        if (apiKey.isNullOrBlank() || apiKey == "null") return null

        val endpoint = "https://safebrowsing.googleapis.com/v4/threatMatches:find" +
        ...
        val body = JSONObject().apply {
            ...
            put("threatInfo", JSONObject().apply {
                put("threatTypes", org.json.JSONArray().apply {
                    put("MALWARE")
                    put("SOCIAL_ENGINEERING")
                    put("UNWANTED_SOFTWARE")
                    put("POTENTIALLY_HARMFUL_APPLICATION")
                })
                put("platformTypes", org.json.JSONArray().apply {
                    put("ANY_PLATFORM")
                })
                put("threatEntryTypes", org.json.JSONArray().apply {
                    put("URL")
                })
                put("threatEntries", org.json.JSONArray().apply {
        ...
        val threatType = matches.optJSONObject(0)?.optString("threatType").orEmpty()
        return ThreatCheckResult(
            checkedUrl = url,
            isMalicious = true,
            isSuspicious = true,
            threatType = threatType,
            message = "This URL may be dangerous",
            source = "Google Safe Browsing"
        )
    }

    suspend fun expandUrl(originalUrl: String): ExpandedUrlResult = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            ExpandedUrlResult(originalUrl = clean, expandedUrl = expanded, success = true)
        } catch (e: Exception) {
            ExpandedUrlResult(
                originalUrl = clean,
                expandedUrl = clean,
    ...
    suspend fun checkAndExpand(url: String): Pair<ThreatCheckResult, ExpandedUrlResult> =
        withContext(Dispatchers.IO) {
            val expanded = expandUrl(url)
            val target = if (expanded.success && expanded.expandedUrl.isNotBlank())
                expanded.expandedUrl else url
